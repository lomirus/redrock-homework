...
func GetComments(userId string) ([]Comment, error) {
	var comments []Comment
	query, err := db.Query("SELECT `id`, `user_id`,`parent_id`,`children_id`,`secret_target`, `value`, `likes`" +
	...
	for query.Next() {
		...
		err := query.Scan(&newComment.Id, &newComment.UserId, &newComment.ParentId, &newComment.ChildrenId,
			&newComment.SecretTarget, &newComment.Value, &newComment.Likes)
		if err != nil {
			return comments, err
		}
		if newComment.SecretTarget == -1 || strconv.Itoa(newComment.SecretTarget) == userId {
...
func GetCommentById(id string) (Comment, error) {
	var comment Comment
	row := db.QueryRow("SELECT `id`, `user_id`,`parent_id`,`children_id`, `secret_target`, `value`, `likes`"+
		" FROM `comments` WHERE id=?", id)
	err := row.Scan(&comment.Id, &comment.UserId, &comment.ParentId, &comment.ChildrenId, &comment.SecretTarget,
...
func AddComment(value string, userId int, secretTargetStr string) error {
	if secretTargetStr == "" {
		...
		secretTarget, err := strconv.Atoi(secretTargetStr)
		if err != nil {
			return err
		}
		_, err = db.Exec("INSERT INTO `comments` (`value`,`user_id`,`secret_target`) VALUES (?,?,?)",
			value, userId, secretTarget)
		if err != nil {
			return err
		}
	}
